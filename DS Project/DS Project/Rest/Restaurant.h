...
private:
	...
	int NFOrders; // number of finished normal orders
	int GFOrders; // number of finished vegan orders 
	int VFOrders; // number of finished VIP orders
	...
	LinkedPQueue<Order*> inServiceGOrders; // priority queue of all in-service vegan orders with the ones that will be finished first dequeued first 
	LinkedPQueue<Order*> inServiceVOrders; // priority queue of all in-service VIP orders with the ones that will be finished first dequeued first 
	*/
...
public:
	...
	void prepareOrders(); // in phase 1: prepares 1 order from each type // in phase 2: assigns relevant orders to available cooks
	void finishOrders(); // in phase 1: finishes 1 order from each type every 5 timesteps // in phase 2: finishes the relevant orders
	...
	void addWaitingVIPOrder(Order*& toBeAddedOrder, double priority);

	bool removeWaitingNOrder(Order*& toCancelOrder, Order*& cancelledOrder);
	bool removeInserviceNOrder(Order*& toCancelOrder);
	
	bool isEmptyRest(); // checks if all events have been executed and all lists are empty

	void printRestInfo(int timeStep);
	void moveToAvailable(int currentTimestep,Cook* breakCook);
